...
export async function getDocsisStatus(
  ...
  try {
    ...
    logger.error(`Could not fetch docsis status from modem.`, inspect(error));
    throw error;
  } finally {
    await modem.logout();
  }
...
`,
  ...
  static flags = {
    ...
    file: Flags.boolean({
      char: "f",
      description:
        "write out a report file under ./reports/${CURRENT_UNIX_TIMESTAMP}_docsisStatus.json",
    }),
    web: Flags.boolean({
      char: "w",
  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      if (flags.web) {
        this.log(
          `Review your docsis state online -> ${webDiagnoseLink(docsisStatus)}`
        );
      }
